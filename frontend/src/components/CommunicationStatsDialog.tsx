...
import { AlertCircle, CalendarDays, ChartNoAxesCombined, ChevronDown, ChevronRight, Clock3, MessageCircle, Send, Users } from "lucide-react";
...
export function CommunicationStatsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [expandedCallInstances, setExpandedCallInstances] = useState<Set<string>>(new Set());
  ...
  const callInstances = stats?.instances.filter(instance => instance.callCount > 0) ?? [];
  const toggleCallInstance = (instanceId: string) => setExpandedCallInstances(current => {
    ...
    <DialogContent className="max-w-[1100px] w-[95vw] h-[90vh] overflow-y-auto p-0 gap-0">
      ...
      <div className="p-5 space-y-6">
        ...
        {stats && <>
          ...
          <section><h3 className="font-semibold mb-3">{t("stats.calls_by_instance")}</h3><div className="rounded-xl border overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/50"><tr><th className="text-left p-3">{t("stats.instance")}</th><th className="text-right p-3">{t("stats.duration")}</th></tr></thead><tbody>{callInstances.map(instance => {
            const expanded = expandedCallInstances.has(instance.providerInstanceId);
            const callContacts = stats.contacts.filter(contact => contact.providerInstanceId === instance.providerInstanceId && contact.callCount > 0).sort((a, b) => b.callDurationSecs - a.callDurationSecs || b.callCount - a.callCount);
            return <Fragment key={instance.providerInstanceId}><tr className="border-t cursor-pointer hover:bg-muted/40 transition-colors" onClick={() => toggleCallInstance(instance.providerInstanceId)} aria-expanded={expanded}><td className="p-3"><span className="flex items-center gap-2">{expanded ? <ChevronDown className="h-4 w-4 shrink-0" /> : <ChevronRight className="h-4 w-4 shrink-0" />}<ProtocolIcon protocol={instance.providerId} size={18} /><span className="font-medium">{instance.instanceName}</span></span></td><td className="p-3 text-right">{formatDuration(instance.callDurationSecs)}</td></tr>{expanded && <tr className="border-t bg-muted/20"><td colSpan={2} className="p-0"><div className="px-5 py-2"><table className="w-full"><thead><tr className="text-xs text-muted-foreground"><th className="text-left py-2">{t("contact")}</th><th className="text-right py-2">{t("stats.duration")}</th></tr></thead><tbody>{callContacts.map(contact => <tr key={`${contact.metaContactId}:${contact.providerInstanceId}`} className="border-t border-border/60"><td className="py-2"><span className="flex items-center gap-2"><Avatar className="h-7 w-7"><AvatarImage src={contact.avatarUrl} /><AvatarFallback className="text-[10px]">{contact.displayName.slice(0,2).toUpperCase()}</AvatarFallback></Avatar><span>{contact.displayName}</span></span></td><td className="py-2 text-right">{formatDuration(contact.callDurationSecs)}</td></tr>)}</tbody></table></div></td></tr>}</Fragment>;
          })}</tbody></table>{callInstances.length === 0 && <div className="p-6 text-center text-sm text-muted-foreground">{t("stats.no_calls")}</div>}</div></section>
          <section><h3 className="font-semibold mb-3">{t("stats.by_contact")}</h3><div className="rounded-xl border overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/50"><tr><th className="text-left p-3">{t("contact")}</th><th className="text-left p-3">{t("stats.instance")}</th><th className="text-right p-3">{t("stats.total")}</th><th className="text-right p-3">{t("stats.sent")}</th><th className="text-right p-3">{t("stats.received")}</th></tr></thead><tbody>{stats.contacts.slice(0, visibleContacts).map(contact => <tr key={`${contact.metaContactId}:${contact.providerInstanceId}`} className="border-t"><td className="p-3"><span className="flex items-center gap-2"><Avatar className="h-8 w-8"><AvatarImage src={contact.avatarUrl} /><AvatarFallback>{contact.displayName.slice(0,2).toUpperCase()}</AvatarFallback></Avatar><span className="font-medium">{contact.displayName}</span></span></td><td className="p-3"><span className="flex items-center gap-2"><ProtocolIcon protocol={contact.providerId} size={18} />{contact.instanceName}</span></td><td className="p-3 text-right font-medium">{contact.total}</td><td className="p-3 text-right">{contact.sent}</td><td className="p-3 text-right">{contact.received}</td></tr>)}</tbody></table></div>{visibleContacts < stats.contacts.length && <div className="text-center mt-3"><Button variant="outline" onClick={() => setVisibleContacts(value => value + 10)}><ChevronDown className="h-4 w-4 mr-1" />{t("show_more")}</Button></div>}</section>
